...
// TODO: 2022/11/23 基本的想法是，在初始化棋子对象的时候，就可以把棋子的图片打印在anchorPane上了，这个时候打印的图片都是背面朝上的，但是由于对象确定，每个棋子实际上是有区别的。但是棋子的图像并没有放在对应的网格中，就是缺一步setX，用ArrayList可以遍历setX。在父类中加入set坐标方法
    ...
    {
        int t = 10;
        if (mode == 1 || (mode == 3 && UserStatus.AISide == 1)||(mode == 4 && UserStatus.AISide == 1)||(mode == 2 && onSide == 0))
        {
        ...
        else if ((mode == 3 && UserStatus.AISide == 0)||(mode == 4 && UserStatus.AISide == 0)||(mode == 2 && onSide == 1))
        {
